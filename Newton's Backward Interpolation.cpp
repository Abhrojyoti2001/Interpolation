...
main()
{
	...
	if (get==1)
		...
		scanf("%f",&X);
		/*Generating a Backward Deference table*/
		for(j=1;j<n;j++)
			{for(i=j;i<n;i++)
			y[i][j]=y[i][j-1]-y[i-1][j-1];}
		/*Printing the Backward Deference table*/
		...
		printf("Newton’s Backward interpolation formula is not applicable because the arguments are not equally spaced.");
}
